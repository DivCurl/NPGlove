...
void npAnimation::Set( rgbw_t color, uint16_t brt ) {
    ...
    for ( int i = 0; i < ( FB_SIZE / 4 ) - 4; i++ ) {
        pDisplay->frameBuffer[ i*4 ] = ( color.g * ( scaledBrt + 1 ) ) >> 8;
        pDisplay->frameBuffer[ i*4 + 1 ] = ( color.r * ( scaledBrt + 1 ) ) >> 8;
        pDisplay->frameBuffer[ i*4 + 2 ] = ( color.b * ( scaledBrt + 1 ) ) >> 8;
        pDisplay->frameBuffer[ i*4 + 3 ] = ( color.w * ( scaledBrt + 1 ) ) >> 8;        
    }
}
